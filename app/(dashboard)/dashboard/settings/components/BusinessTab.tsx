    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2 italic text-yellow-600">Nom Commercial Override</label>
          <input
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Devise du Menu</label>
          <input
      ...
      <div className="space-y-4">
        <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Description du Business</label>
        <textarea
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Horaires (JSON)</label>
          <textarea
      ...
      <div className="space-y-4">
        <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Réseaux sociaux</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          { key: "show_prices", label: "Afficher les prix", icon: "💰" },
          { key: "show_images", label: "Afficher les images", icon: "🖼️" },
        ].map((item) => (
            ...
            <button
              onClick={() =>
                setNestedField(
                  "business",
                  item.key,
                  !formData.business?.[item.key as keyof typeof formData.business]
                )
              }
              className={`w-12 h-6 rounded-full transition-all ${formData.business?.[item.key as keyof typeof formData.business] ? "bg-yellow-500" : "bg-slate-200"}`}
            >
              <div className={`w-4 h-4 bg-white rounded-full mx-1 transition-transform ${formData.business?.[item.key as keyof typeof formData.business] ? "translate-x-6" : ""}`} />
            </button>
          </div>
        ))}
      </div>
