...
export default function TenantStatisticsPage() {
  ...
  const { screens, loading } = useTVStream(params?.tenantId);
  ...
  const offlineCount = useMemo(() => screens.filter((s) => !s.is_online).length, [screens]);
  ...
  const audioSchedules = useMemo(
    () => schedules.filter((s) => !!s.audio_track_id).length,
    [schedules]
  );
    ...
    <div dir={isRtl ? "rtl" : "ltr"} className="p-4 sm:p-6 md:p-8 space-y-6 dashboard-shell rounded-3xl">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="dashboard-surface p-5">
          <p className="text-xs uppercase font-black text-slate-400 flex items-center gap-2"><Tv size={14} /> {text.screens}</p>
          <p className="mt-2 text-3xl font-black text-slate-900">{screens.length}</p>
        </div>
        <div className="dashboard-surface p-5">
          <p className="text-xs uppercase font-black text-slate-400 flex items-center gap-2"><Activity size={14} /> {text.online}</p>
          <p className="mt-2 text-3xl font-black text-emerald-600">{onlineCount}</p>
        </div>
        <div className="dashboard-surface p-5">
          <p className="text-xs uppercase font-black text-slate-400 flex items-center gap-2"><Clock3 size={14} /> {text.totalTime}</p>
          <p className="mt-2 text-3xl font-black text-slate-900">{formatHours(totalUptime)}</p>
        </div>
        <div className="dashboard-surface p-5">
          <p className="text-xs uppercase font-black text-slate-400 flex items-center gap-2"><Monitor size={14} /> {text.average}</p>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="dashboard-surface p-5 col-span-2 lg:col-span-1">
          <p className="text-xs uppercase font-black text-slate-400 flex items-center gap-2"><Music2 size={14} /> {text.audioSlots}</p>
          ...
          <div className="mt-4 space-y-2 max-h-[360px] overflow-auto">
            ...
            {categoryParution.map((row) => (
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{text.activeProductsCount}: {row.productsCount}</p>
                </div>
                <span className="font-black text-slate-900">{row.scheduleCount} {text.slots}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="dashboard-surface p-5">
          ...
          <div className="mt-4 space-y-2 max-h-[360px] overflow-auto">
            ...
            {productParution.slice(0, 25).map((row) => (
                ...
                <span className="font-black text-slate-900">{row.parutionCount} {text.slots}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {screens.map((screen) => (
            <div key={screen.id} className="rounded-xl border border-slate-200 p-3">
              ...
              <p className="text-xs text-slate-500 mt-1">{text.template}: {screen.current_template || "standard"}</p>
              <p className="text-xs text-slate-500 mt-1">{text.uptime}: {formatHours(screen.total_uptime_hours || 0)}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {loading && (
        <div className="dashboard-surface p-3 text-sm font-semibold text-slate-600">
          {text.syncing}
        </div>
      )}
    </div>
